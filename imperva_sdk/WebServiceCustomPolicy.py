...
class WebServiceCustomPolicy(MxObject):
  ...
  def __new__(Type, *args, **kwargs):
    obj_exists = WebServiceCustomPolicy._exists(connection=kwargs['connection'], Name=kwargs['Name'])
    if obj_exists:
      return obj_exists
    else:
  ...
  def _exists(connection=None, Name=None):
    for cur_obj in connection._instances:
      if type(cur_obj).__name__ == 'WebServiceCustomPolicy':
  ...
  def __init__(self, connection=None, Name=None, Enabled=None, Severity=None, Action=None, FollowedAction=None, SendToCd=None, DisplayResponsePage=None, ApplyTo=[], MatchCriteria=[], OneAlertPerSession=None):
    super(WebServiceCustomPolicy, self).__init__(connection=connection, Name=Name)
  ...
  #
  # Web Service Custom Policy Parameters
  #
  @property
  def Name(self):
  ...
  def FollowedAction(self, FollowedAction):
    if FollowedAction != self._FollowedAction:
      self._connection._update_web_service_custom_policy(Name=self._Name, Parameter='followedAction', Value=FollowedAction)
      self._FollowedAction = FollowedAction
  @Action.setter
  def Action(self, Action):
    if Action != self._Action:
      self._connection._update_web_service_custom_policy(Name=self._Name, Parameter='action', Value=Action)
      self._Action = Action
  @Severity.setter
  ...
  def SendToCd(self, SendToCd):
    if SendToCd != self._SendToCd:
      self._connection._update_web_service_custom_policy(Name=self._Name, Parameter='sendToCD', Value=SendToCd)
      self._SendToCd = SendToCd
  ...
  def DisplayResponsePage(self, DisplayResponsePage):
    if DisplayResponsePage != self._DisplayResponsePage:
      self._connection._update_web_service_custom_policy(Name=self._Name, Parameter='displayResponsePage', Value=DisplayResponsePage)
      self._DisplayResponsePage = DisplayResponsePage
  @OneAlertPerSession.setter
  def OneAlertPerSession(self, OneAlertPerSession):
    if OneAlertPerSession != self._OneAlertPerSession:
      self._connection._update_web_service_custom_policy(Name=self._Name, Parameter='oneAlertPerSession', Value=OneAlertPerSession)
      self._OneAlertPerSession = OneAlertPerSession
  ...
  def ApplyTo(self, ApplyTo):
    ...
    if change:
      self._connection._update_web_service_custom_policy(Name=self._Name, Parameter='applyTo', Value=change)
      self._ApplyTo = MxList(ApplyToObjects)
    
  #
  # Web Service Custom Policy internal functions
  #
  @staticmethod
  def _get_all_web_service_custom_policies(connection):
    res = connection._mx_api('GET', '/conf/webServiceCustomPolicies')
    try:
      policy_names = res['customWebPolicies']
    except:
      raise MxException("Failed getting Web Service Custom Policies")
    policy_objects = []
  ...
  @staticmethod
  def _get_web_service_custom_policy(connection, Name=None):
    validate_string(Name=Name)
    obj_exists = WebServiceCustomPolicy._exists(connection=connection, Name=Name)
    if obj_exists:
      return obj_exists
    try:
  ...
  @staticmethod
  def _create_web_service_custom_policy(connection, Name=None, Enabled=None, Severity=None, Action=None, FollowedAction=None, SendToCd=None, DisplayResponsePage=None, ApplyTo=None, MatchCriteria=None, OneAlertPerSession=None, update=False):
    validate_string(Name=Name)
    ...
    if pol:
      ...
      connection._mx_api('POST', '/conf/webServiceCustomPolicies/%s' % Name, data=json.dumps(body))
      return WebServiceCustomPolicy(connection=connection, Name=Name, Enabled=Enabled, Severity=Severity, Action=Action, FollowedAction=FollowedAction, SendToCd=None, DisplayResponsePage=DisplayResponsePage, ApplyTo=ApplyToObjects, MatchCriteria=MatchCriteria, OneAlertPerSession=OneAlertPerSession)
  @staticmethod
  def _delete_web_service_custom_policy(connection, Name=None):
    validate_string(Name=Name)
    pol = connection.get_web_service_custom_policy(Name=Name)
    if pol:
      connection._mx_api('DELETE', '/conf/webServiceCustomPolicies/%s' % Name)
