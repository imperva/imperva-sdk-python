...
class DataType(MxObject):
  '''
  MX global object data type Class
  Note that we don't support updating/setting

  >>> dataType = mx.get_data_type("testDataType")
  >>> dataType.rules
  ...
  >>> mx._create_data_type(**dataTypeDict)
  <imperva_sdk 'DataType' Object - 'user defined - testDataType'>
  ...
  def __new__(Type, *args, **kwargs):
    obj_exists = DataType._exists(connection=kwargs['connection'], Name=kwargs['Name'])
    if obj_exists:
      return obj_exists
    else:
  ...
  def _exists(connection=None, Name=None):
    for cur_obj in connection._instances:
      if type(cur_obj).__name__ == 'DataType':
  ...
  def __init__(self, connection=None, Name=None, IsSensitive=True, Rules=[], TargetTableGroupName=None):
    super(DataType, self).__init__(connection=connection, Name=Name)
  ...
  def Rules(self):
    ...
    >>> rule = mx.get_data_type("testDataType")
    >>> rule.Rules
    [
        {
            "name": "testRule1",
  ...
  def IsSensitive(self, IsSensitive):
    if IsSensitive != self._IsSensitive:
      self._connection._update_data_type(Name=self._Name, Parameter='IsSensitive', Value=IsSensitive)
      self._IsSensitive = IsSensitive
  ...
  def Rules(self, Rules):
    ...
    tmp2 = sorted(tmp2)
    if tmp1 != tmp2:
      self._connection._update_data_type(Name=self._Name, Parameter='Rules', Value=Rules)
      self._Rules = Rules
  ...
  def TargetTableGroupName(self, TargetTableGroupName):
    if TargetTableGroupName != self._TargetTableGroupName:
      self._connection._update_data_type(Name=self._Name, Parameter='TargetTableGroupName',
                                         Value=TargetTableGroupName)
      self._TargetTableGroupName = TargetTableGroupName


  #
  # Data type internal functions
  #

  @staticmethod
  def _get_all_data_type(connection):
  ...
  @staticmethod
  def _get_data_type_by_name(connection, Name=None):
    validate_string(Name=Name)
    obj_exists = DataType._exists(connection=connection, Name=Name)
    if obj_exists:
      return obj_exists
    try:
      ...
      return None
    return DataType(connection=connection, Name=Name, IsSensitive=res['sensitive'], Rules=res['rules'],
                    TargetTableGroupName=res['target-table-group-name'])

  @staticmethod
  def _create_data_type(connection, Name=None, IsSensitive=True, Rules=[], TargetTableGroupName=None, update=False):
    validate_string(Name=Name)
    obj = connection.get_data_type(Name=Name)
    if obj:
      if not update:
        raise MxException("Data type '%s' already exists" % Name)
      else:
    ...
    try:
      ...
      raise MxException("Failed creating data type: %s" % e)

    return DataType(connection=connection, Name=Name, IsSensitive=IsSensitive, Rules=Rules,
                    TargetTableGroupName=TargetTableGroupName)

  @staticmethod
  def _update_data_type(connection, Name=None, Parameter=None, Value=None):
    # Data type doesn't support update
