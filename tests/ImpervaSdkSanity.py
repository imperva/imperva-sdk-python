###########################################################################################################
#
# This unittest runs a basic sanity for imperva-sdk that should ensure most things are working properly -
# 1) Create resources
# 2) Export to JSON
# 3) Delete resources
# 4) Import from JSON
...
class TestImpervaSdkSanity(unittest.TestCase):
  ...
  Site			= {"Name": "imperva-sdk sanity site"}
  ServerGroup		= {"Name": "imperva-sdk sanity server group", "Site": Site["Name"], "ProtectedIps": [], "OperationMode": "active"}
  WebService		= {"Name": "imperva-sdk sanity web service", "ServerGroup": ServerGroup["Name"], "Site": Site["Name"]}
  WebApplication	= {"Name": "imperva-sdk sanity web application", "WebService": WebService["Name"], "ServerGroup": ServerGroup["Name"], "Site": Site["Name"], "Mappings": [{ "priority": 1, "host": "www.myapp.com", "hostMatchType": "Exact" }]}

  ActionSet		= {"Name": "imperva-sdk sanity action set", "AsType": "security"}
  Action		= {
                            "Name": "imperva-sdk sanity action", 
                            "ActionType": "GWSyslog", 
                            "Port": "514", 
                            "Host": "syslog-server", 
                            "Protocol": "TCP", 
  ...
  WebServiceCustomPolicy = {
                            "Name": "imperva-sdk sanity web service custom policy",
  ...
  ParameterTypeGlobalObject = {
				"Name": "imperva-sdk sanity parameter type configuration",
				"Regex": ".*"
			}

  HttpProtocolSignaturesPolicy = {
                            "Name": "imperva-sdk sanity http protocol signatures policy", 
                            "ApplyTo": [{'siteName': Site["Name"], 'serverGroupName': ServerGroup["Name"], 'webServiceName': WebService["Name"]}],
  ...
  def test_sdk_license(self):
    ...
    if self.license:
      mx = imperva-sdk.MxConnection(Host=self.host, Username=self.user, Password=self.password, Port=self.port, FirstTime=True)
      try:
  ...
  def test_sdk_sanity(self):
    ...
    mx = imperva-sdk.MxConnection(Host=self.host, Username=self.user, Password=self.password, Port=self.port)
    ...
    except imperva-sdk.MxExceptionNotFound:
